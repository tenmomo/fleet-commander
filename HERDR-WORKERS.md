...
To pin a worker identity or per-seat environment, pass `--env KEY=VALUE` on `workspace create` and verify from inside — have the pane shell echo the variable to a file (`ps eww` cannot read another process's environment on macOS). Measured 2026-07-22 with `PI_CODING_AGENT_DIR` selecting a second Pi subscription: the footer's `(sub)` tag cannot distinguish two subscriptions, so the env echo is the only identity proof. 补充(2026-07-26):当 Pi 的 statusline 扩展已装时,footer 下方那行会直接写出订阅名(例如 `★ profile-b·7d·9%·↻6.5d`)——比 env 回显更硬且免费,出自 Pi 进程自身而非壳层。**先看这一行,拿不到再回落到 env 回显。**⚠ 该 marker 对 openrouter 等 **API-key 通路整个隐藏**(另一指挥席同日四席实测),所以 env 回显的回落路径**不能删**——只有订阅制通路才有这一行。同一屏还能一并验模型与思考等级(`(openai-codex) gpt-5.6-sol • high`)。 When hand-launching a worker in a bare-shell pane (`pane send-text`), do NOT `exec` the harness — `exec pi` makes worker exit close the pane, the tab, and (if last tab) the whole workspace (2026-07-23: a double-Ctrl-C seat reset vaporized both seats and their workspace); launch bare (`pi …`) so exit returns to the shell and the seat survives for relaunch. **Done** = exact pane id held, the requested worker process running, any hook-trust dialog resolved, and no launch error on the pane.
