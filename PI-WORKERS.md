...
## Pi 当指挥席:没有 `/goal`、压缩后不自续(2026-08-23 实测,一次指挥席交接,gpt-5.6-sol max)

Pi 无内建 `/goal`/`/loop`(SLASH-COMMANDS 表),auto-compact 后回到空编辑器**不会自己续航**。把「不停手」做成壳层守护(前任席或 cron 跑):每 60s 读 `herdr agent list` 的 `agent_status`,**idle/blocked/done 三态都算**(`done` 是 Pi 压缩后与 turn 结束的常态,漏它守护就是聋的)、pane 正显 `Compacting context` 时不推;持续 ≥8 min 才 `herdr agent prompt` 一句指针(指向它自己压缩前写进 SEAT 的 COMPACT-SURVIVAL 锚),上限 6 次。停机协议:指挥官判定真队列空 → SEAT **行首**写 `QUEUE-EMPTY <时间> <原因>`;守护 `grep -E '^\**QUEUE-EMPTY[ *:]'` 只认行首——v1 用裸 `grep QUEUE-EMPTY`,被描述句里的同字样当场误触发退出(检测器先验:先喂已知正例/负例)。
